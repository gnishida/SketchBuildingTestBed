...
void CGA::derive(const std::map<std::string, Grammar>& grammars, const std::map<std::string, Grammar>& default_grammars, bool final_view, bool suppressWarning) {
	shapes.clear();
	...
	while (!stack.empty()) {
		...
		// if no grammar is found but final view is selected, the system selects a default grammar
		if (!found && final_view) {
			for (auto it = default_grammars.begin(); it != default_grammars.end(); ++it) {
				if (it->second.contain(shape->_name)) {
					default_found = true;
					name = it->first;
					break;
		...
		if (found || default_found) {
			...
			if (!final_view && shape->_grammar_type != grammars.at(name).type) {
			...
			if (found) {
				...
				shape->_grammar_type = default_grammars.at(name).type;
				default_grammars.at(name).getRule(shape->_name).apply(shape, default_grammars.at(name), stack, shapes);
			}
		} else {
			if (!suppressWarning && shape->_name.back() != '!' && shape->_name.back() != '.') {
				std::cout << "Warning: " << "no rule is found for " << shape->_name << "." << std::endl;
