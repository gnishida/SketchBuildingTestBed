...
void main(){
	...
	// lighting
	if (lighting == 1) {
		intensity = 0.5 + (visibility * 0.95 + 0.05) * 0.5 * max(0.0, dot(-lightDir, varyingNormal));

		def_intensity = vec3(0.5, 0.5, 0.5) + (visibility * 0.95 + 0.05) * diffuse;
		def_diffuse = (ambient + (visibility * 0.95 + 0.05) * diffuse) * def_diffuse;
	}
	else {
		intensity = 0.5 + (visibility * 0.95 + 0.05) * 0.5;

		def_intensity = vec3(0.5, 0.5, 0.5) + (visibility * 0.95 + 0.05) * vec3(1, 1, 1);
		def_diffuse = (ambient + (visibility * 0.95 + 0.05)) * def_diffuse;
	}
